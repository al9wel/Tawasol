...
registerBtn.addEventListener("click", async () => {
    let { value: info } = await Swal.fire({
        ...
        <div>
            <input accept="image/*" onchange="selectImages('swall-input6','postImage','delBtnProfile')" type="file" id="swall-input6">
            <label  for="swall-input6" class="up-btn p-2 rounded-md">اختيار صوره</label>
        </div>
        <div style="width: 80%; margin: 10px auto; display: flex; justify-content: center;align-items: center; flex-direction: column-reverse; gap: 10px;">
            <button onclick="delPic('swall-input6','postImage','delBtnProfile')" id="delBtnProfile" style="color: #ffffff;background:rgb(38, 50, 56);" class="rounded-full p-2 hidden cursor-pointer">حذف</button>
            <img id="postImage" style="width: 150px;height: 150px;object-fit: cover;border: 1px solid white;" class="rounded-full hidden" src="" alt="">
        </div>
        `,
        focusConfirm: false,
        preConfirm: () => {
